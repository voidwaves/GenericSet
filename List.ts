...
export namespace List {
    ...
    // return a boolean, depending on an element existing in a list
    export function includes<Type>([head, ...tail]: Type[], value: Type, status = true): boolean {
        return ((head !== undefined) && status) ? includes(tail, value, status && (head === value)) : status
    }
